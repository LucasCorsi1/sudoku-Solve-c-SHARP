                ...
                else
                {
                    for (int i = 0; i < 9; ++i)
                    {
                        ...
                        {   /* Se ele entrar neste if significa que o número pode ser inserido na matriz */
                            ...
                            Thread temporizador1 = new Thread(proMostrar);
                            temporizador1.Start();
                            temporizador1.Join();
                            Thread.Sleep(2);
                             if (linha + 1 < 9)
                            {   /* Se a linha for de 0 a 8 ele irá tentar a resolução na próxima linha */
                                ...
                                {   /*Caso a resolução do if não dê certo significa que ele precisa pegar o próximo então ele irá zerar a variavel*/
                                    ...
                                    Thread temporizador2 = new Thread(proMostrar);
                                    temporizador2.Start();
                                    temporizador2.Join();
                                    Thread.Sleep(2);
                                }
                            ...
                            else if(coluna + 1 < 9)
                            {
                                ...
                                { /*Caso a resolução do if não dê certo significa que ele precisa pegar o próximo então ele irá zerar a variavel*/
                                    ...
                                    Thread temporizador3 = new Thread(proMostrar);
                                    temporizador3.Start();
                                    temporizador3.Join();
                                    Thread.Sleep(2);
                                    sudoku[linha, coluna] = 0;
                                ...
                                if (splitLines.Length == 9)
                                {
                                    for (int i = 0; i < splitLines.Length; i++)
                                    {
                                        if (int.Parse(splitLines[i]) >= 0 && int.Parse(splitLines[i]) <= 9)
                                        {
                                            ...
                                            {
                                                sudoku[i, j] = 0;
                                                dataGridSudoku.Rows[i].Cells[j].Style.ForeColor = Color.Red;
                                            }
                                            else
                                            {
                                                dataGridSudoku.Rows[i].Cells[j].Value = splitLines[i];
                                                dataGridSudoku.Rows[i].Cells[j].Style.ForeColor = Color.Black;
